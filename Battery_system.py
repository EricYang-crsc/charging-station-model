...
class BatterySystem:
    ...
    所有参数动态从 battery_params 模块读取，修改参数文件后无需修改本文件
    """
    ...
    def __init__(self, technology=None):
        ...
        """
        # ---------- 从参数模块动态读取所有参数 ----------
        self.capacity = Battery_params.BATTERY_CAPACITY
        self.power = Battery_params.BATTERY_POWER

        # 技术路线选择
        if technology is not None:
            self.technology = technology
        else:
            self.technology = Battery_params.BATTERY_TECHNOLOGY

        # 根据技术路线选择对应的单价
        if self.technology == "交流":
            self.cost_per_wh = Battery_params.BATTERY_COST_PER_WH_AC
        elif self.technology == "直流":
            self.cost_per_wh = Battery_params.BATTERY_COST_PER_WH_DC
        else:
            raise ValueError(f"未知的技术路线：{self.technology}，请选择'交流'或'直流'")

        self.cell_cost_per_wh = Battery_params.BATTERY_CELL_COST_PER_WH

        # 设备性能
        self.efficiency = Battery_params.BATTERY_EFFICIENCY
        self.dod = Battery_params.BATTERY_DOD
        self.cycle_life = Battery_params.BATTERY_CYCLE_LIFE
        self.degradation = Battery_params.BATTERY_DEGRADATION
        self.lifetime = Battery_params.BATTERY_LIFETIME

        # 投资与成本
        self.opex_rate = Battery_params.BATTERY_OPEX_RATE

        # 折旧与残值
        self.cell_dep_years = Battery_params.BATTERY_CELL_DEPRECIATION_YEARS
        self.cell_residual_rate = Battery_params.BATTERY_CELL_RESIDUAL_RATE
        self.other_dep_years = Battery_params.BATTERY_OTHER_DEPRECIATION_YEARS
        self.other_residual_rate = Battery_params.BATTERY_OTHER_RESIDUAL_RATE

        # 运行策略
        self.cycle_per_day = Battery_params.BATTERY_CYCLE_PER_DAY
        self.operating_days = Battery_params.BATTERY_OPERATING_DAYS
